...
- **Don't reintroduce NOT NULL / PARTITION BY on `tenantId`** for Reservations/Payments/Customers — their FK is `ON DELETE SET NULL`, so the column must stay nullable. (Migration `20260718000004/006/007` backfill NULLs to default tenant id=1.) // codacy-suppress Agentlinter_security_no-pii-exposure
