...
export const test = base.extend({
  loginAsPlatformAdmin: [async ({ page }, use) => { await use(loginAsPlatformAdmin(page)); }, { auto: true }],
  loginAsTenantStaff: [async ({ page }, use) => { await use(loginAsTenantStaff(page)); }, { auto: true }],
});

export { E2E_SUPER_ADMIN_EMAIL, E2E_SUPER_ADMIN_PASSWORD, E2E_TENANT_SLUG, E2E_TENANT_EMAIL, E2E_TENANT_PASSWORD };
