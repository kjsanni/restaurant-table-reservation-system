...
function createReq(user = { id: 1 }, body = {}, params = {}, tenant = null) {
  return { user: { ...user, tenant }, tenant, body, params, ip: "127.0.0.1" };
}
...
describe("marketplace.controller", () => {
  ...
  it("update accepts whitelisted fields and strips non-whitelisted fields", async () => {
    ...
    const req = createReq({ id: 1, tenant: { id: 1 } }, { title: "Updated", position: 2, maliciousField: "ignored" }, { id: "1" }, { id: 1 });
    const res = createRes();
    await updateListingHandler(req, res);
    expect(res.status).toHaveBeenCalledWith(200);
    expect(marketplaceDAO.updateListing).toHaveBeenCalledWith("1", expect.objectContaining({ title: "Updated", position: 2 }), 1);
    expect(marketplaceDAO.updateListing).toHaveBeenCalledWith("1", expect.not.objectContaining({ maliciousField: "ignored" }), 1);
  });

  it("update returns 404 when not found", async () => {
    marketplaceDAO.updateListing.mockResolvedValue(null);
    const req = createReq({ id: 1, tenant: { id: 1 } }, { title: "Updated" }, { id: "999" }, { id: 1 });
    const res = createRes();
    await updateListingHandler(req, res);
  ...
  it("remove returns 404 when not found", async () => {
    ...
    const req = createReq({ id: 1, tenant: { id: 1 } }, {}, { id: "999" }, { id: 1 });
    const res = createRes();
