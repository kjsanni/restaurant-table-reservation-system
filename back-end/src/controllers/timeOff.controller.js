...
const getTimeOffsHandler = async (req, res) => {
  ...
  const items = await timeOffService.getTimeOffs(status, req.tenant?.id);
  return res.status(200).json({ success: true, timeOffs: items });
};

const getStaffHandler = async (req, res) => {
  const staff = await timeOffService.getAllStaff(req.tenant?.id);
...
const createHandler = async (req, res) => {
  ...
  const item = await timeOffService.createTimeOff({ userId, startDate, endDate, reason }, req.tenant?.id);
...
const updateStatusHandler = async (req, res) => {
  ...
  const item = await timeOffService.updateTimeOffStatus(id, status, req.tenant?.id);
  return res.status(200).json({ success: true, timeOff: item });
};

const deleteHandler = async (req, res) => {
  const { id } = req.params;
  const result = await timeOffService.deleteTimeOff(id, req.tenant?.id);
