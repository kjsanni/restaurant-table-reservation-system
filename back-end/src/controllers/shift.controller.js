...
const getShiftsHandler = async (req, res) => {
  ...
  const shifts = await shiftService.getShiftsByDay(dayOfWeek);
  return res.status(200).json({ success: true, shifts });
};

const getStaffHandler = async (req, res) => {
  const staff = await shiftService.getAllStaff();
...
const createShiftHandler = async (req, res) => {
  ...
  const shift = await shiftService.createShift({ userId, dayOfWeek, startTime, endTime, role });
