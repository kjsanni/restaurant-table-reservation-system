...
passSigningRequestDAO.create = async ({
  ...
  return await db.passSigningRequest.create({
    tenantId,
    eventId,
    requesterId,
    designSnapshot,
...
passSigningRequestDAO.listByTenant = (tenantId, filters = {}) => {
  ...
  return db.passSigningRequest.findAll({
    where,
...
passSigningRequestDAO.updatePaymentStatus = async (id, paymentReference) => {
  const request = await db.passSigningRequest.findByPk(id);
...
passSigningRequestDAO.updatePlatformStatus = async (id, platform, status) => {
  ...
  await request.update({ platformStatuses: current });
  return request;
};
...
passSigningRequestDAO.setSigning = async (id) => {
  const request = await db.passSigningRequest.findByPk(id);
  if (!request) return null;
  await request.update({ status: "signing" });
  return request;
};
...
passSigningRequestDAO.markCompletedIfAllDone = async (id) => {
  ...
  if (allSigned) {
    ...
    await request.update({ status: "failed", completedAt: new Date() });
  }
  return request;
};
