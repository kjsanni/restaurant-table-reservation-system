...
const getEventBookingAttributes = (DataTypes) => ({
  ...
  paymentReference: { type: DataTypes.STRING(255), allowNull: true },
...
module.exports = (sequelize, DataTypes) => {
  class EventBooking extends Model {
    static associate(models) {
      if (models.tenant) {
        EventBooking.belongsTo(models.tenant, { foreignKey: "tenantId", onDelete: "SET NULL" });
      }
      if (models.event) {
        EventBooking.belongsTo(models.event, { foreignKey: "eventId", onDelete: "CASCADE" });
      }
      if (models.ticketType) {
        EventBooking.belongsTo(models.ticketType, { foreignKey: "ticketTypeId", onDelete: "SET NULL" });
      }
      if (models.customer) {
        EventBooking.belongsTo(models.customer, { foreignKey: "customerId", onDelete: "SET NULL" });
      }
      if (models.user) {
        EventBooking.belongsTo(models.user, { foreignKey: "userId", onDelete: "SET NULL" });
      }
    }
  }

  EventBooking.init(getEventBookingAttributes(DataTypes), {
    sequelize,
